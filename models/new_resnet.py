...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, index, stride=1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(inplanes, index[0], kernel_size=1, bias=True)
        self.bn1 = BatchNorm2d_no_b(planes)
        self.conv2 = nn.Conv2d(index[0], index[1], kernel_size=3, stride=stride,
                               padding=1, bias=True)
        self.bn2 = BatchNorm2d_no_b(index[1])
        self.conv3 = nn.Conv2d(block_index[1], index[2], kernel_size=1, bias=True)
        self.bn3 = BatchNorm2d_no_b(index[2])
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, unit_index, blocks, stride=1):
        downsample = None
        
        if stride != 1 or self.inplanes != unit_index[0]:
